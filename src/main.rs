...
impl State {
    ...
    fn render_mene(&mut self, ctx: &mut BTerm, title: &str) {
        ctx.cls();
        ctx.print_centered(5, title);
        ctx.print_centered(8, "(P) playing");
    ...
    fn dead(&mut self, ctx: &mut BTerm) {
        self.render_mene(ctx, "you are dead")
    }
}
